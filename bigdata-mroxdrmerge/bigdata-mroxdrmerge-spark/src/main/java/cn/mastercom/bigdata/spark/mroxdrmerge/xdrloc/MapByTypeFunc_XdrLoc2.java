...
public class MapByTypeFunc_XdrLoc2 extends MapByTypeFuncBaseV2 implements org.apache.spark.api.java.function.PairFlatMapFunction<Iterator<Tuple2<Long,Tuple4<Iterable<String>,Iterable<String>,Iterable<String>,Iterable<String>>>>,TypeInfo, Iterable<String>>
{
	...
	protected int init_once_sub() throws Exception
	{
		...
		{
			LOGHelper.GetLogger().writeDetailLog(LogType.error,"xdr init_once_sub error","xdr init_once_sub error : " +
                    "mme: " +
                    mmeSplitMax + "http: "
                    + httpSplitMax, e);
		}
...
//					LOGHelper.GetLogger().writeLog(LogType.info, "ottdata:" +strs.length+ "->" + locData);
					...
					if (strs[0].equals("URI") || strs[0].equals(""))
					{
						...
						{
							LOGHelper.GetLogger().writeDetailLog(LogType.error,"error ottdata1", "error ottdata:" +
									locData, e);
						}
					}
				...
				{
					LOGHelper.GetLogger().writeDetailLog(LogType.error, "DataMark_Location Error","DataMark_Location Error: ", e);
				}
			}
			...
			while(it3.hasNext())
			{
				...
				{
					LOGHelper.GetLogger().writeDetailLog(LogType.error,"get lastXdrData Error", "get lastXdrData Error: ", e);
				}
			}
			...
			while(it1.hasNext())
			{
				...
				{
					LOGHelper.GetLogger().writeDetailLog(LogType.error,"DataMark_MME Error", "DataMark_MME Error: ", e);
				}
			...
			while(it2.hasNext())
			{
				...
				{
					LOGHelper.GetLogger().writeDetailLog(LogType.error,"DataMark_HTTP Error", "DataMark_HTTP Error: ", e);
				}
			...
			{
				LOGHelper.GetLogger().writeDetailLog(LogType.error,"pushData Error", "pushData Error: ", e);
			}
		}
