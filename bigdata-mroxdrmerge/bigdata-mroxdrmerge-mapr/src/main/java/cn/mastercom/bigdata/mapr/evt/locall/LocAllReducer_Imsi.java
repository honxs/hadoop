	...
	public static class StatReducer_Imsi extends DataDealReducerV2<ImsiKey, Text, NullWritable, Text> {
		...
		protected void setup(Context context) throws IOException, InterruptedException {
			...
			try {
				...
				LOGHelper.GetLogger().writeLog(LogType.error,"data format warm", "data format warm : "+e.getMessage(), e);
			}
			...
			try{
				...
				while((line=br.readLine())!=null){
					try{
						...
						LOGHelper.GetLogger().writeLog(LogType.warn,"collectImeiMobileTypeFillDataWarn", "collectImeiMobileTypeFillDataWarn : "
										+e.getMessage(), e);
					}
				}
			}catch (Exception e){
				LOGHelper.GetLogger().writeLog(LogType.warn,"collectImeiMobileTypeMapError", "collectImeiMobileTypeMapError : "
						+e.getMessage(), e);
			}
		}
