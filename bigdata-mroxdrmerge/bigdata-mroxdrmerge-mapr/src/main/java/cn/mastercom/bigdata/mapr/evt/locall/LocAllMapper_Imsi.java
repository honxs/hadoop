...
public class LocAllMapper_Imsi
{
	...
	public static class UserLocMapper_XDRLOC extends DataDealMapperV2<Object, Text, ImsiKey, Text>
	{
		...
		{  
			...
			{
				LOGHelper.GetLogger().writeLog(LogType.error,"loclib parsing Exception", "loclib parsing Exception：" + locString, e);
	...
	public static class XdrDataMapper_Imsi extends DataDealMapperV2<Object, Text, ImsiKey, Text>
	{
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException
		{
			...
			{
				LOGHelper.GetLogger().writeLog(LogType.error,"locAllLocLib.fillData error", "locAllLocLib.fillData error : " + value.toString(), e);
				return;
			}
		...
		private XdrDataBase initDataType(String dirPath) throws InterruptedException
		{
			...
			{
				LOGHelper.GetLogger().writeLog(LogType.error, "locAllMapper_Imsi Map Error","mergeType: " + mergeType +
                        " " +
                        "formatPath: " +
                        formatPath,e);
	...
	public static class ResidentUserMapper extends DataDealMapperV2<Object, Text, ImsiKey, Text>{
		...
		protected void map(Object key, Text value, Context context)
            ...
            try{
                ...
                LOGHelper.GetLogger().writeLog(LogType.error,"ResidentUserMapper filldata error", "ResidentUserMapper filldata error: ",e);
            }
