...
public class EventDataBuildCellGridStat extends BaseStatDo
{
	...
	public EventDataBuildCellGridStat(int stime, int etime, int resultTBName, ResultOutputer resultOutputer)
	{
		this.stime = stime;
		this.etime = etime;
	...
	public int statSub(Object o)
	{
		...
		tmStr = event.iEci + "," + event.iBuildID+","+event.iHeight;
	...
	public int outDealingResultSub()
	{
		for (EventDataBuildCellGrid item : dataMap.values())
		{
			...
			{
				LOGHelper.GetLogger().writeLog(IWriteLogCallBack.LogType.error,"EventDataBuildCellGridStat.outDealingResultSub Exception",
						"EventDataBuildCellGridStat.outDealingResultSub Exception: " + e.getMessage(),e);
			}
		}
	...
	public int outFinalReusltSub()
	{
		for (EventDataBuildCellGrid item : dataMap.values())
		{
			...
			{
				LOGHelper.GetLogger().writeLog(IWriteLogCallBack.LogType.error,"EventDataBuildCellGridStat.outFinalReusltSub Exception",
						"EventDataBuildCellGridStat.outFinalReusltSub Exception: " + e.getMessage(),e);
			}
		}
		dataMap = new HashMap<String, EventDataBuildCellGrid>();	
		return 0;
	...
	public class EventDataBuildCellGrid extends BaseEventDataStatDo
	{
	    ...
	    public EventDataBuildCellGrid(int iCityID, int iBuildingID, int iHeight, long iECI, int iTime)
	    {
	    	super();
