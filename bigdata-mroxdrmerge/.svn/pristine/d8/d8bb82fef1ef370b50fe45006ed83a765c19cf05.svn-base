...
//					reader = new BufferedReader(new InputStreamReader(hdfsOper.getHdfs().open(new Path(filePath)), "UTF-8"));
				...
				while ((strData = reader.readLine()) != null)
				{
					...
					{
						e.printStackTrace();
						LOGHelper.GetLogger().writeLog(LogType.error,"loadBuildIdCell error4",
								"loadBuildIndoorCellUser error : " + strData, e);
			...
			{
				e.printStackTrace();
				LOGHelper.GetLogger().writeLog(LogType.error,"loadBuildIndoorCellUser", "loadBuildIndoorCellUser error ", e);
		...
		{
			e.printStackTrace();
			LOGHelper.GetLogger().writeLog(LogType.error, "loadBuildIndoorCellUser error","loadBuildIndoorCellUser error ", e);
			errLog = "loadBuildIndoorCellUser error : " + e.getMessage();
			return false;
		}
