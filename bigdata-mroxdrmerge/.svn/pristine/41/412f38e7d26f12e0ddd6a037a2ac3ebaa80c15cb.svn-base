		...
		protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			try {
				...
				if (MainModel.GetInstance().getCompile().Assert(CompileMark.Debug)) {
					LOGHelper.GetLogger().writeLog(LogType.error,"mme format error", "mme format error：" + xmString, e);
				}
			}
		}
	}
	...
	public static class EciFilterMapper_HTTP extends DataDealMapperV2<Object, Text, EciFilterKey, Text> {
		...
		protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			try {
				...
				if (MainModel.GetInstance().getCompile().Assert(CompileMark.Debug)) {
					LOGHelper.GetLogger().writeLog(LogType.error,"http format error", "http format error：" + xmString, e);
				}
			}
		}
	}
	...
	public static class EciFilterMapper_XDRLOCATION extends DataDealMapperV2<Object, Text, EciFilterKey, Text> {
		...
		protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			try {
				...
				if (MainModel.GetInstance().getCompile().Assert(CompileMark.Debug)) {
					LOGHelper.GetLogger().writeLog(LogType.error,"xdrlocation format error", "xdrlocation format error：" + value, e);
				}
			}
		}
	}
