...
public class UepWork  implements IWork
{
	...
	public void init() throws Exception
	{
		...
		ConfigHelper conf = new ConfigHelper("config/app_uep.xml");
	...
	{
		// yyyyMMdd
		List<File> dirs = IOHelper.getDirs(path.getAbsolutePath(), "\\d{8}$");
		if (dirs == null || dirs.size() == 0) return null;
	...
	{
		List<File> files = IOHelper.getFiles(dir.getAbsolutePath(), "感知覆盖.*\\.txt$", true);
		...
		return new UepTask(this, dir, files);
	}
