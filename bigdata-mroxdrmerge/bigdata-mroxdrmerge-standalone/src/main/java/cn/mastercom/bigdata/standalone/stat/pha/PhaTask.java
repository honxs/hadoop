...
public class PhaTask extends ATask
{

	private Collection<File> m_files;
	private PhaWork m_Work;
	private File m_dir;

	public PhaTask(PhaWork work, File dir, Collection<File> files)
	{
		m_Work = work;
		m_dir = dir;
		m_files = files;
	...
	public List<Exception> run()
	{
		...
		FileTypeResult fileTypeResult = new FileTypeResult(m_Work.dstPath, dirName + sdf.format(now), dateStr, PhaEnum.values());
	...
	private void doOne(final PhaStatDeals statDeals, File file, final DataAdapterReader dataAdapterReader) throws Exception
	{
	...
	private void parse(DT_Sample_4G sample, DataAdapterReader reader) throws ParseException
	{
		...
		sample.wtimems = (short) reader.GetIntValue("Wtimems", 0);
